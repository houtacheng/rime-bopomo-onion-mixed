...
- 預設輸出繁體中文。
- 候選右側顯示對應簡體提示。
- 選字時按 `Shift + →`，直接將目前輸入鍵碼輸出為純注音符號。
- 選字時按 `Command + →`，直接輸出目前候選的簡體版本。
- 輸入「今年、去年、明年、今天、昨天、明天、現在、時區」，顯示動態日期與時間格式。
  - 原文字詞維持第 1 候選；第 2、3 候選為 `YYYY.MM.DD` 與 `YYYY.MM.DD(日)`（括號內依實際星期變化）。
- 輸入公里、公分、星座等名稱，可選擇對應的單位或特殊符號。
- 按 `Ctrl + \\` 後輸入阿拉伯數字，可轉換為中文數字、金融大寫、蘇州碼、羅馬數字及圈點數字。
- 所有轉換與個人詞頻學習都在本機完成。
