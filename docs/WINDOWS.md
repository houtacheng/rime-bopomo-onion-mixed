...
以下在 macOS 上驗證過、Windows 上沒有，第一次安裝後請留意：

1. **`Alt + ←`（英文輸出）** —— Windows 的檔案總管與瀏覽器用它做「上一頁」。
   組字中輸入法通常優先，但不是每個程式都一樣。若被攔走，改綁別的鍵。
2. **`Control + →`（拼音輸出）** —— Windows 沒有 macOS 那個「切換桌面空間」的衝突，
   理論上比 macOS 更順，但部分編輯器用它做「跳一個單字」。
3. **`Control` + 數字（輸出數字）** —— 只在未組字時作用，與選字用的 `Control+1~6` 不衝突。

Windows 上反而少了兩個 macOS 特有的坑：沒有 Command 鍵被前端丟棄的問題，
也沒有系統快捷鍵搶走 `Control + →` 的問題。
